defmodule Week6.Printer do
  ...
  def handle_cast({:print_tweet, chunk}, state) do
    ...
    if success == :ok do
      ...
      SentAnalyser6.calculate_sentiment(tweet_id, text)

      EngageCalculator6.calculate_engagement(tweet_id, {favorites, retweets, followers, name, userID})

      Redacter6.filter_bad_words(tweet_id, text)

      if hashtags != nil do
        Week6.StatMaker.send_hashtags(hashtags)
      end
