defmodule Week6.Aggregator do
  ...
  # state is %{ "uuid" : %{ engagement: 0, sentiment: 0, cleaned: "", userID: 1545, name: ""}}
  ...
  def handle_cast({:size, batch_size}, state) do
    ...
    {_, state} =
      Enum.reduce_while(state, {0, state}, fn tuple, acc ->
        ...
        {counter, state_dict} =
          if length(Map.keys(aggregatedTweet)) == 6 do
            Week6.Batcher.send_to_batch(aggregatedTweet)
            counter = elem(acc, 0)
  ...
  def handle_cast({:agg, message}, state) do
    {_tweet_id, state} =
      case message do
        {:engagement, result} ->
          ...
          aggregatedTweet = Map.put(aggregatedTweet, :userID, result.userID)
          aggregatedTweet = Map.put(aggregatedTweet, :userName, result.name)
          aggregatedTweet = Map.put(aggregatedTweet, :tweetID, result.id)
    ...
    # if length(Map.keys(aggregated)) == 6 do
    #   # Week6.Batcher.send_to_batch(aggregated)
    # end
