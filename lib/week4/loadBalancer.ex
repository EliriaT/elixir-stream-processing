defmodule Week4.LoadBalancer do
  ...
  def print_tweets(chunk) do
    [{_, numberOfPools}] = :ets.lookup(:loadBal,:length)
    loadBal = :rand.uniform(numberOfPools)
    [{_, uuid}] = :ets.lookup(:loadBal, loadBal)

    GenServer.cast(String.to_atom(uuid), {:print_tweet, chunk})
  end
  ...
  def handle_cast({:print_tweet, chunk}, state) do
    pidMap = elem(state, 2)
    {workerID1, loadsMap} = getIDByLeastRequestLB(elem(state, 1), pidMap)

    Week4.PrinterSupervisor.sendToPrinterX(
      pidMap.my_id,
      pidMap.printerSuperPid,
      workerID1,
      chunk
    )

    # {workerID2, loadsMap} = getIDByLeastRequestLB(loadsMap)
    # Week4.PrinterSupervisor.sendToPrinterX(workerID2, chunk)

    {:noreply, {workerID1, loadsMap, elem(state, 2)}}
  end

  # Least Request Load Balancing
  def getIDByLeastRequestLB(nodeLoads, pidMap) do
    sum =
